...
// 아이템의 정체성은 **이름**이다. 캐시가·아이콘·마일가능은 사용자가 고치는 속성이고,
// 이름을 바꾸면 다른 아이템으로 본다(이 한계는 문서화한다 — 두 기기가 각자 이름을 고치면 갈라진다).
export const MY_ITEM_ID_FIELDS = [["name", "s"]];

// 아이템 삭제 표식은 거래 삭제 표식과 **같은 맵**(`ledger.deleted`)에 네임스페이스를 붙여 넣는다.
// 이유: (1) DB 스키마·JSONB 형태를 바꾸지 않는다, (2) 표식을 모르는 구버전 탭도 키를 그대로 보존해
// 업로드하므로 삭제가 전파되고 자가 치유된다, (3) TTL·개수 상한·병합 로직을 그대로 재사용한다.
