...
### B-2c. 게스트는 tombstone 이 영원히 쌓인다 — LOW
TTL 정리는 클라우드 병합 시 서버 시각으로만 한다(기기 시계를 믿지 않기 위해).
로그인하지 않는 게스트는 병합이 없어 표식이 계속 남는다. 삭제 1건당 약 40바이트라
실사용에서 문제되지 않지만, 상한(예: 최신 N개 유지)을 두는 편이 안전하다.
