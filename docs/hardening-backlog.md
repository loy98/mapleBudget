...
- **삭제 표식**: `ledger.deleted` 안에 `item:<id>` 네임스페이스로 산다. DB 스키마·JSONB 형태를 바꾸지 않고,
  표식을 모르는 구버전 탭도 키를 그대로 보존해 업로드하므로 삭제가 전파된다(자가 치유).
...
**남은 한계**: 두 기기가 같은 아이템의 이름을 각자 다르게 고치면 서로 다른 id 가 되어 둘 다 남는다
(이름이 정체성이라는 모델의 대가). 설정(`calc`)은 스칼라 묶음이라 LWW 를 유지한다.
